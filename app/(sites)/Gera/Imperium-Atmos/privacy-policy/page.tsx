...
export default function PrivacyPolicy() {
  const { site } = useSite();
  const { privacyPolicy } = getSiteConfig(site);

  return (
    <div className="flex-1 flex flex-col gap-4 p-4">
      <div className="flex flex-col gap-1 px-2 lg:px-8 border-b pb-2">
        <h1 className="text-xl font-bold text-center">{privacyPolicy.title}</h1>
        <p className="text-sm text-center">{privacyPolicy.description}</p>
      </div>
